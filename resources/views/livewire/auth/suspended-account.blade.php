...
<div class="flex flex-col space-y-6">
    ...
    <div class="bg-[#F8EBD5] dark:bg-zinc-800/50 border border-[#FFF200] dark:border-yellow-400 rounded-xl p-6">
        <div class="flex items-center mb-3">
            ...
            <div class="ml-3">
                <h3 class="text-lg font-semibold text-[#231F20] dark:text-white">Account Temporarily Suspended</h3>
            </div>
        </div>
    ...
    @if($canSendAppeal)
        ...
        <div class="space-y-6">
            <div>
                <h2 class="text-xl font-semibold text-[#231F20] dark:text-white mb-2">Submit an Appeal</h2>
            ...
            @if(session('success'))
                <div class="bg-[#F8EBD5] dark:bg-zinc-800/50 border border-[#FFF200] dark:border-yellow-400 rounded-xl p-4">
                    <div class="flex items-center">
                        ...
                        <p class="text-[#231F20] dark:text-white text-sm">{{ session('success') }}</p>
                    </div>
                </div>
            @endif
            ...
            <form wire:submit="sendAppeal" class="space-y-6">
                <div>
                    ...
                    <textarea 
                        ...
                        id="message"
                        rows="6" 
                        class="w-full px-4 py-3 border border-[#9B9EA4] dark:border-zinc-600 rounded-xl focus:ring-2 focus:ring-[#FFF200] focus:border-transparent resize-none dark:bg-zinc-800/50 dark:text-white dark:placeholder-zinc-400"
                ...
                <button 
                    ...
                    class="w-full bg-[#FFF200] dark:bg-yellow-400 hover:bg-[#FFF200]/90 dark:hover:bg-yellow-300 text-[#231F20] dark:text-zinc-900 font-semibold py-3 px-6 rounded-xl transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg hover:shadow-xl"
                >
    ...
    @else
        ...
        <div class="bg-[#F8EBD5] dark:bg-zinc-800/50 border border-[#FFF200] dark:border-yellow-400 rounded-xl p-6">
            @if($lastAppeal)
                <h3 class="text-lg font-semibold text-[#231F20] dark:text-white mb-3">Appeal Status</h3>
                <div class="space-y-3">
                    <p class="text-[#231F20] dark:text-zinc-300">
                    ...
                    <div class="flex items-center space-x-2">
                        <strong class="text-[#231F20] dark:text-white">Status:</strong>
                        @if($lastAppeal->status === 'pending')
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-[#FFF200] dark:bg-yellow-400 text-[#231F20] dark:text-zinc-900">
                                Under Review
                            </span>
                            <span class="text-[#231F20] dark:text-zinc-300">Your appeal is being reviewed by our administrators.</span>
                        @elseif($lastAppeal->status === 'reviewed')
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-[#9B9EA4] dark:bg-zinc-600 text-white">
                                Reviewed
                            </span>
                        @elseif($lastAppeal->status === 'approved')
                            ...
                            <span class="text-green-700 dark:text-green-400">Your appeal has been approved. Please try logging in again.</span>
                        @elseif($lastAppeal->status === 'rejected')
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 dark:bg-red-900/50 text-red-800 dark:text-red-400">
                                Rejected
                            </span>
                        @endif
                    </div>
                    ...
                    @if($lastAppeal->admin_response)
                        <div class="bg-white dark:bg-zinc-800/50 border border-[#9B9EA4] dark:border-zinc-600 rounded-lg p-4 mt-3">
                            <p class="text-sm font-medium text-[#231F20] dark:text-white mb-1">Administrator Response:</p>
                            <p class="text-[#9B9EA4] dark:text-zinc-400 text-sm">{{ $lastAppeal->admin_response }}</p>
                        </div>
                    @endif
                </div>
            @endif
            ...
            @if($hoursUntilNextAppeal > 0)
                <div class="mt-4 p-3 bg-[#FFF200]/20 dark:bg-yellow-400/20 border border-[#FFF200] dark:border-yellow-400 rounded-lg">
    ...
    <div class="flex flex-col sm:flex-row gap-4">
        <button 
            wire:click="logout"
            class="flex-1 bg-[#9B9EA4] dark:bg-zinc-600 hover:bg-[#9B9EA4]/90 dark:hover:bg-zinc-500 text-white font-semibold py-3 px-6 rounded-xl transition-all duration-200 shadow-lg hover:shadow-xl"
        >
            Back to Login
        </button>
        ...
        <a 
            ...
            class="flex-1 bg-white dark:bg-zinc-800/50 border border-[#9B9EA4] dark:border-zinc-600 hover:bg-[#F8EBD5] dark:hover:bg-zinc-700/50 text-[#231F20] dark:text-white font-semibold py-3 px-6 rounded-xl transition-all duration-200 text-center shadow-lg hover:shadow-xl"
        >
